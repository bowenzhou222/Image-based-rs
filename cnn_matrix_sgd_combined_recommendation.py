...
class Recommendation:
    def __init__(self, training_file, test_file, feature_file, test_items, training_items, recommendation_list_length, raw_item_number, f):
    ...
    def sgd(self):
        self.weight_vectors = []
        for dim in range(self.factor_number):
            weights = [0]*4096
            for iteration in range(50):
                # for item_id in self.item_id_set:
                for item_index in range(self.item_number):
                    # item_index = self.item_id_dictionary[item_id]
                    feature = self.training_item_features[item_index]
                    predicted_factor = np.dot(feature, weights)
                    factor = self.item_factor_matrix[item_index][dim]
                    error = factor - predicted_factor

                    weights += 0.01*error*feature
                    '''for i in range(4096):
                        w = weights[i]
                        x = feature[i]
                        w += 0.01*error*x'''
        ...
        for item_id in self.test_item_id_set:
            ...
            feature = self.test_item_features[item_index]
            i = 0
            for weights in self.weight_vectors:  # each dimension
                factor = np.dot(feature, weights)
                self.test_item_factor_matrix[item_index][i] = factor
                i += 1
        # print(self.test_item_factor_matrix[0])
    ...
    def hit_ratio(self, recommendation_user_number):
        ...
        print('CNN SGD:\t'+str(number_in_recommendation_list) + '\t' + str(total_consumed_item_number) + '\t' +
